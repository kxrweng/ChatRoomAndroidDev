...
//    }
    ...
    private Boolean isValidSignUpDetails(){
        if(encodedImage == null){
            ...
            Pattern patternPassword = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{6,}$");
            Matcher matcherPassword = patternPassword.matcher(binding.inputPassword.getText().toString().trim());
